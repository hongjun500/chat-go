...
func (w *wsConn) SendEnvelope(m *Envelope) error {
...
func (ws *WebSocketServer) Start(ctx context.Context, addr string, gateway Gateway, opt Options) error {
	if ws.Codec == nil {
		ws.Codec = &JSONCodec{} // default to JSON
	}
	if ws.Path == "" {
		ws.Path = "/ws"
	}
...
func (ws *WebSocketServer) handleLegacyMessage(sess *wsConn, text string, gateway Gateway) {
	...
	if client.Name == "" {
		payload, _ := json.Marshal(SetNamePayload{Name: text})
		envelope := &Envelope{
			Type:    "set_name",
			Payload: payload,
			Ts:      time.Now().UnixMilli(),
	...
	if len(text) > 0 && text[0] == '/' {
		payload, _ := json.Marshal(CommandPayload{Raw: text})
		envelope := &Envelope{
			Type:    "command",
			Payload: payload,
			Ts:      time.Now().UnixMilli(),
	...
	payload, _ := json.Marshal(ChatPayload{Content: text})
	envelope := &Envelope{
		Type:    "chat",
		From:    client.Name,
		Payload: payload,
...
// Helper function to get client from WebSocket session
func getWSClient(sess Session) *chat.Client {
