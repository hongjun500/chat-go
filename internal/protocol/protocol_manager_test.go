...
func TestProtocolManager_BackwardCompatibility(t *testing.T) {
	// Test backward compatible Protocol struct
	p := NewProtocol(CodecJson)
	
	if p.GetCodec() == nil {
		t.Error("Expected non-nil codec from backward compatible interface")
	}
	
	// Test message creation
	textMsg := p.CreateTextMessage("test")
	if textMsg.Type != MsgText {
		t.Errorf("Expected text message type, got %s", textMsg.Type)
	}
	
	ackMsg := p.CreateAckMessage("ok")
